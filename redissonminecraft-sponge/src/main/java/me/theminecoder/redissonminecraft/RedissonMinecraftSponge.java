    ...
    public void onServerStart(GameStartedServerEvent event) {
        ...
        if (this.config.isEnableDynamicServers()) {
            Sponge.getScheduler().createTaskBuilder().async().intervalTicks(200).execute(() -> {
                ...
                RedissonMinecraft.getClient().getMapCache("redissonminecraft__backend-servers").put(config.getServerName(), address.getHostName() + ":" + address.getPort(), 1, TimeUnit.MINUTES);
